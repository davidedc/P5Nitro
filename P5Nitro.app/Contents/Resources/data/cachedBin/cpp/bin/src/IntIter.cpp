...
{
{
	HX_SOURCE_POS("/usr/lib/haxe/std/IntIter.hx",39)
	this->min = min;
	HX_SOURCE_POS("/usr/lib/haxe/std/IntIter.hx",40)
	this->max = max;
}
;
	return null();
...
bool IntIter_obj::hasNext( ){
	...
	HX_SOURCE_POS("/usr/lib/haxe/std/IntIter.hx",47)
...
int IntIter_obj::next( ){
	...
	HX_SOURCE_POS("/usr/lib/haxe/std/IntIter.hx",54)
	return (this->min)++;
}
